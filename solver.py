...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            print("##########################Save Model##########################")
            torch.save(self.model.state_dict(), os.path.join(path, str(self.dataset) + '_checkpoint_finally.pth'))
...
#############################################################################################
    def test(self):
        self.model.load_state_dict(
            torch.load(
                os.path.join(str(self.model_save_path), str(self.dataset) + '_checkpoint_finally.pth')))
        self.model.eval()
        temperature = 50
...
###########################################################################################
            path=os.path.join("/data2/NTNU/MAAT/checkpoints/pre", str(i) + '.pdf')
            input = input.float().detach().cpu().numpy()
            output=output.float().detach().cpu()
            # gt = np.concatenate((input[0, :, -1], labels[0, :, -1]), axis=0)
            # pd = np.concatenate((input[0, :, -1], output[0, :, -1]), axis=0)
            gt = np.concatenate((input[0, :, -1], input[0, :, -1]), axis=0)
            pd = np.concatenate((input[0, :, -1], output[0, :, -1]), axis=0)
            visual(gt, pd,path )
...
#############################################################################################
        results1 = np.vstack([gt, pred, test_energy]).T
        results1 = pandas.DataFrame(results1)
        results1.columns = ['ground_truth', 'pred', 'test_criterion']
        results1.to_csv("/data2/NTNU/MAAT/checkpoints/pre/" + 'result.csv')
...
###############################################################################
        ...
        # 保存混淆矩阵图
        conf_matrix_path = os.path.join("/data2/NTNU/MAAT/checkpoints/pre", "confusion_matrix.png")
